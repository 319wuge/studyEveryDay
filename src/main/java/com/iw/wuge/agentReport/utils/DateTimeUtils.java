...
public class DateTimeUtils {
	...
	public static String firstDateOfWeek(String date) {
		return firstDateOfWeek(strDate2Date(date));
	}

	public static String firstDateOfWeek(Date date) {
		Calendar calendar = date2Calendar(date);
		calendar.setFirstDayOfWeek(Calendar.MONDAY);
		int weekIndex = calendar.get(Calendar.DAY_OF_WEEK);
		if (weekIndex == 2)
			return date2StrDate(date);
		//+1 表示从周一开始算第一天，周天算最后一天
		return date2StrDate(DateUtil.addDay(date, 1 - weekIndex + 1));
	}

	public static String lastDateOfWeek(String date) {
		return lastDateOfWeek(strDate2Date(date));
	}

	public static String lastDateOfWeek(Date date) {
		Calendar calendar = date2Calendar(date);
		calendar.setFirstDayOfWeek(Calendar.MONDAY);
		int weekIndex = calendar.get(Calendar.DAY_OF_WEEK);
		if (weekIndex == 1)
			return date2StrDate(date);
		//+1 表示从周一开始算第一天，周天算最后一天
		return date2StrDate(DateUtil.addDay(date, 7 - weekIndex + 1));
	}

	public static void main(String[] args) {
		System.out.println(getWeekChinese(new Date()));
		Calendar calendar = date2Calendar(DateTimeUtils.strDate2Date("2017-06-12"));
		System.out.println(lastDateOfWeek("2017-06-12"));
	}
}
